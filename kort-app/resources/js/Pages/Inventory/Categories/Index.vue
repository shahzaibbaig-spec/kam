...
const applyFilters = () => {
    router.get(route('inventory.categories.index'), filters, {
        preserveState: true,
        replace: true,
    });
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <PageHeader
                ...
                description="Organize burn center consumables into practical supply groups for fast store workflows, safer batch control, and clean reporting later."
                action-label="Add Category"
                :action-href="route('inventory.categories.create')"
            />
        </template>
                    ...
                    <table class="min-w-full divide-y divide-slate-200 text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <tr v-for="category in categories.data" :key="category.id">
                                ...
                                <td class="py-4 text-slate-700">{{ category.items_count }}</td>
                                <td class="py-4">
